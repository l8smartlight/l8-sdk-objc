   ...
   /* [self clearMatrixWithSuccess:^{
    ...
    for(int i=0;i<[[self.t userPeripherals] count];i++){
        ...
        if([[[[self.t userPeripherals] objectAtIndex:i] valueForKey:@"isUserChecked"] isEqualToString:@"YES"]){
            ...
            [self.t L8SL_SendData:Cmd toL8SL:activePeripheralUser withSuccess:^{
            ...
            [self.t L8SL_SendData:Cmd2 toL8SL:activePeripheralUser withSuccess:^{
...
- (void)setSuperLED:(UIColor *)color withSuccess:(L8VoidOperationHandler)success failure:(L8JSONOperationHandler)failure
{
    ...
    const CGFloat  *componentts = CGColorGetComponents(color.CGColor);
                ...
                if (!state ) {
                    ...
                    for(int i=0;i<[[self.t userPeripherals] count];i++){
                        ...
                        if([[[[self.t userPeripherals] objectAtIndex:i] valueForKey:@"isUserChecked"] isEqualToString:@"YES"]){
                            ...
                            [self.t L8SL_SendData:Cmd toL8SL:activePeripheralUser ];
                            [self.t L8SL_SendData:Cmd2 toL8SL:activePeripheralUser ];
